...
export default function AdminEditBannerPage() {
  ...
  const fetchBanner = async () => {
    try {
      const response = await bannersAPI.adminGetById(bannerId);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await bannersAPI.update(bannerId, payload);
      toast.success('Banner berhasil diperbarui');
      router.push('/admin/banners');
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Gagal memperbarui banner');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <h1 className="text-xl font-semibold text-gray-900 mb-6">Edit Banner</h1>
